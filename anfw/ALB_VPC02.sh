...
# Create ALB
ALB_VPC02_ARN=$(aws elbv2 create-load-balancer \
    --name $ALB_NAME \
...
echo "ALB ARN: $ALB_VPC02_ARN"

# Get Target Group ARN
TARGET_GROUP02_ARN=$(aws elbv2 describe-target-groups --names $TARGET_GROUP_NAME --query "TargetGroups[0].TargetGroupArn" --output text)
echo "Target Group ARN: $TARGET_GROUP02_ARN"

# Create Listener
aws elbv2 create-listener \
    --load-balancer-arn $ALB_VPC02_ARN \
    --protocol HTTP \
    --port 80 \
    --default-actions Type=forward,TargetGroupArn=$TARGET_GROUP02_ARN
